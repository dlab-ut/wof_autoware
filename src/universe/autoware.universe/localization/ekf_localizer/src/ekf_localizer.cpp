  ...
  twist_queue_(params_.twist_smoothing_steps)
{
  ...
  if (params_.publish_tf_) {
    timer_tf_ = rclcpp::create_timer(
      ...
      RCLCPP_INFO(get_logger(), "publisher activated_: %s", timer_tf_ ? "true" : "false");
  }
...
void EKFLocalizer::timer_callback()
{
  ...
  // RCLCPP_INFO(get_logger(), "timer_call_back is_activated_: %s", is_activated_ ? "true" : "false");
  ...
  const geometry_msgs::msg::TwistStamped & current_ekf_twist)
{
  ...
  odometry.child_frame_id = "base_link";//base_linkからodomに変更
